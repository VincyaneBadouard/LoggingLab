...
#' - The dead trees under felled trees (DeathCause = "treefall2nd")
#'
#' @param accesspts Random access point of maintrail for each PU (sfc_MULTIPOLYGON)
#'
...
#' - SmoothedTrails: Smoothed main and secondary trails polygons (sfc_MULTIPOLYGON)
#' - MainTrailsAccess : Random access point of maintrail for each PU (sfc_MULTIPOLYGON)
...
#'   advancedloggingparameters = loggingparameters())
#'
#' PostLogInventory <- treefelling(secondtrails$inventory, scenario = "manual", fuel = scenariosparameters$fuel,
#' winching = scenariosparameters$winching, directionalfelling = scenariosparameters$directionalfelling,
#' maintrailsaccess = secondtrails$MainTrailsAccess, scndtrail = secondtrails$SmoothedTrails,
#' advancedloggingparameters = loggingparameters())
#'
...
#'
#'   # 2ndary trails
#'     geom_sf(data = st_as_sf(secondtrails$SmoothedTrails), aes(color = "Initial-trails"),alpha = 0.5) +
#'     geom_sf(data = st_as_sf(secondtrails$RawSecondTrails), color = "green",alpha = 0.5) +
#'
#'   # 2ndary trails adjusted
#'     geom_sf(data = st_as_sf(ScdTrailsAdj$SmoothedTrails), aes(color = "Adjusted-trails"),alpha = 0.5) +
#'     geom_sf(data = st_as_sf(ScdTrailsAdj$RawSecondTrails), color = "red",alpha = 0.5) +
#'
#'     scale_colour_manual(values = c("Visible defect" = "pink",
#'     "Harvestable" = "skyblue",
#'   "HarvestableUp" = "blue", "Selected" = "red", "Future" = "orange",
#'   "Reserve" = "purple", "Probed hollow" = "forestgreen", "Harvestable area" = "olivedrab",
#'   "Initial-trails" = "darkgreen" , "Adjusted-trails" = "darkred"))
#'
...
secondtrailsadjusted <- function(
  ...
  HarvestableTreesPoints <- PostLogInventory %>%
  ...
  SelectedTreesPoints <- PostLogInventory %>%
    filter(Selected == "1")
  ...
  PostLogInventory_sf <- PostLogInventory %>%
  ...
  PostLogInventory_Tr <- PostLogInventory %>%  getgeometry(TreePolygon)

  Trunks <- st_cast(PostLogInventory_Tr$TreePolygon , "POLYGON")[seq(1, by = 2, len = nrow(PostLogInventory))]


  Trunks <- Trunks %>% sf::st_set_crs(sf::st_crs(topography))
  PostLogInventory_Tr <- PostLogInventory_sf

  for (h in 1:nrow(PostLogInventory)) {
    if (!st_is_empty(Trunks[h])) {
      PostLogInventory_Tr$CrownGeom[h] <- st_difference(Trunks[h],PostLogInventory_sf[h,] %>%
                                                            st_buffer(dist = (PostLogInventory_sf$TrunkHeight[h]-0.05))) %>%
  ...
  # Points vector with coordinates of the selected trees:
  SelectedCrownsPoints <- st_as_sfc(PostLogInventory_Tr$CrownGeom) %>%
  ...
  FutureTreesPoints <- PostLogInventory %>%
  ...
  ReserveTreesPoints <- PostLogInventory %>%
  ...
  BigTreesPoints <- PostLogInventory %>%
  ...
  treeselectionoutputs  <- list(inventory = PostLogInventory ,
