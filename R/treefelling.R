...
#'   advancedloggingparameters = loggingparameters())
#'
#' NewInventory <- treefelling(secondtrails$inventory, scenario = "manual", fuel = scenariosparameters$fuel,
...
treefelling <- function(
  ...
  if(!all(unlist(lapply(list(maintrailsaccess), inherits, "sf"))))
    stop("The 'maintrailsaccess' arguments of the 'treefelling' function must be sf")

  if(!all(unlist(lapply(list(scndtrail), inherits, "sfc"))))
    stop("The 'scndtrail' arguments of the 'treefelling' function must be sf")
...
#'      dplyr::dplyr::select(idTree,DBH,TrunkHeight,TreeHeight,CrownHeight,
...
felling1tree <- function(
  ...
  if(!all(unlist(lapply(list(scndtrail), inherits, "sfc"))))
    stop("The 'scndtrail' argument of the 'felling1tree' function must be sf")

  if(!all(unlist(lapply(list(maintrailsaccess), inherits, "sf"))))
    stop("The 'maintrailsaccess' argument of the 'felling1tree' function must be sf")
