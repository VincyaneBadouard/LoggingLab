...
# 
# # Fut/Res crowns/circumference as points
# FutResAllPoints <- list()
# for (FR in 1:nrow(FutureReserveCrowns)) {
#   ## if the fut/res tree is smaller or of the same height than the tree to be felled: we have to avoid the crown too
#   if(FutureReserveCrowns$TreeHeight[FR] <= dat$TreeHeight){
#     
#     FutResAllPoints[[FR]] <- st_cast(st_sfc(FutureReserveCrowns$Crowns[FR]), "POINT", group_or_split = TRUE) # crowns edge in points form
#     
#     ## if the fut/res tree is taller than the tree to be felled: we will only avoid it on its DBH
#   }else if(FutureReserveCrowns$TreeHeight[FR] > dat$TreeHeight){
#     
#     # PROBLEMES de list 
#     FutResAllPoints[[FR]] <- st_point(c(FutureReserveCrowns$Xutm[FR], FutureReserveCrowns$Yutm[FR])) %>%  # the fut/res tree foot point
#       st_buffer(dist = FutureReserveCrowns$DBH[FR]/2) %>%  # create a buffer of DBH diameter
...
# do.call(st_union, FutResAllPoints)

```

## On the data.frame version
```{r}
...
```
## Test time of each version
```{r}
# fun1 <- function(data, H = 30){
#   FutResAllPoints <- list()
#   for (FR in 1:nrow(data)) {
#     ## if the fut/res tree is smaller or of the same height than the tree to be felled: we have to avoid the crown too
#     if(data$TreeHeight[FR] <= H){
#       
#       FutResAllPoints[[FR]] <- st_cast(st_sfc(data$Crowns[FR]), "POINT", group_or_split = TRUE) # crowns edge in points form
#       
#       ## if the fut/res tree is taller than the tree to be felled: we will only avoid it on its DBH
#     }else if(data$TreeHeight[FR] > H){
#       
#       # PROBLEMES de list 
#       FutResAllPoints[[FR]] <- st_point(c(data$Xutm[FR], data$Yutm[FR])) %>%  # the fut/res tree foot point
#         st_buffer(dist = data$DBH[FR]/2) %>%  # create a buffer of DBH diameter
#         st_cast("MULTIPOINT") %>%  # polygon to multipoint
#         st_sfc() %>% 
#         st_cast("POINT", group_or_split = TRUE)
#     }
#   }
#   return(do.call(st_union, FutResAllPoints))
# }
# 
# fun2 <- function(data, H = 30){
#   
#   FutResCrownsPoints <- data %>% 
#     filter(TreeHeight <= H) %>% 
#     summarise(Points = st_combine(Crowns))
#   
#   FutResDBHPoints <- data %>% 
#     filter(TreeHeight > H) %>% 
#     rowwise() %>% 
#     mutate(Points = st_point(c(Xutm, Yutm)) %>% 
#              st_buffer(dist = DBH/2) %>% 
...
#   
#   FutResAllPoints <- bind_rows(FutResCrownsPoints, FutResDBHPoints) %>% 
#     summarise(Points = st_combine(Points))
#   
#   return(st_cast(st_sfc(FutResAllPoints$Points), "POINT", group_or_split = TRUE))
# }
# 
# microbenchmark::microbenchmark(fun1(FutureReserveCrowns), fun2(FutureReserveCrowns), times = 10)
```
...
for (FRP in 1:length(FutResAllPoints)) { # for each point
  ...
  if(FutResDists[[FRP]] < dat$TreeHeight){
    ...
    if(FutResAllPoints[[FRP]][1] >= Foot[1]){ # x FRP > x foot
    ...
    }else if(FutResAllPoints[[FRP]][1] < Foot[1]){ # x FRP < x foot
    ...
    if(TrailAnglesToAvoid[[FRP]] > 1 & TrailAnglesToAvoid[[FRP]] < 179){
      ...
      AnglesToAvoid[[FRP]] <- round(TrailAnglesToAvoid[[FRP]], digits = 0)
