...
NearestPoints <- SelectedTrees %>%
  group_by(idTree) %>% 
  do(geometry = st_tree(., ScndTrail = ScndTrail, MainTrail = MainTrail, fuel = "2", directionalfelling ="2", advancedloggingparameters = loggingparameters())$NearestPoints) %>% 
...
FellingTrees <- SelectedTrees %>% 
  ...
  do(geometry = st_tree(., ScndTrail = ScndTrail, MainTrail = MainTrail, fuel = "2", directionalfelling ="2", advancedloggingparameters = loggingparameters())$A) %>% # inform geometry. # Filling a column from a function whose input is a table
...
get_treepolygon <- function(inventory){
  inventory %>% 
...
st_intersection( # trees under the fallen trees
  ...
  geom_sf(data = get_treepolygon(inventory2), fill = "red")

st_intersection(
  get_treepolygon(inventory2),
