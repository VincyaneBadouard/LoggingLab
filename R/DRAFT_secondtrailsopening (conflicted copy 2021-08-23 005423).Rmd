...
##################################
  ...
  for (accessPtId in 1:dim(PointAcc)[1]) {
    ...
    PointTree <- ptsWIPmax %>% filter(type == "Tree") %>% 
        mutate(ptAccCbl = max.col(-matrix(CostDistEstCbl,ncol = dim(PointAcc)[1] ),ties.method = "first"))
      
    if (dim(PointAcc)[1] == 1) {
      PointTree <- PointTree %>% mutate(EstCostCbl = matrix(CostDistEstCbl,ncol = dim(PointAcc)[1] ))
    }else{
      ...
      if (winching == "2") {
        ...
        PointTree <- PointTree %>% 
          mutate(ptAccGpl = max.col(-t(CostDistEstGrpl),ties.method = "first")) 
        
          if (dim(PointAcc)[1] == 1) {
            PointTree <- PointTree %>% mutate(EstCostGrpl = CostDistEstGrpl)
          }else{
              PointTree <- PointTree %>% mutate(EstCostGrpl = apply(CostDistEstGrpl,1, min))}
...
```{r}
  paths <- do.call(rbind, pathLines)
    ...
    df <-  as_tibble(rasterToPoints(CostRasterMeanGrpl)) %>% mutate( Cost = layer) %>% dplyr::select(-layer)
