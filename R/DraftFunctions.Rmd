...
# LoggingSimulationOutputs <- Rslt
# usethis::use_data(LoggingSimulationOutputs, overwrite = TRUE)

# LoggingSimulationOutputs_iter <- Rslt
# usethis::use_data(LoggingSimulationOutputs_iter, overwrite = TRUE)
...
SpeciesCriteria <- read_csv("D:/VSC Kourou/DATA/SpeciesCriteria.csv",
  ...
  mutate(Aggregative = ifelse(CommercialName == "Angélique" | CommercialName == "Gonfolo rose",
                              TRUE, FALSE))
# unite(Genus, Species, col = "ScientificName", sep = "_", remove = F)

SpeciesCriteria$CommercialName <- tolower(SpeciesCriteria$CommercialName)
...
# usethis::use_data(HarvestableAreaOutputsCable, overwrite = TRUE)
```

# Secondary trails in Data
```{r}
scenario <- "manual"
winching <- "2"
objective <- 10
...
treeselectionoutputs <- suppressWarnings(treeselection(inventory,
                                                       topography = DTMParacou,
                                                       speciescriteria = SpeciesCriteria, objective = objective,
                                                       scenario = scenario, fuel = fuel,
                                                       diversification = diversification,
                                                       winching = winching, specieslax = FALSE,
                                                       objectivelax = TRUE, plotslope = HarvestableAreaOutputsCable$PlotSlope,
                                                       harvestablepolygons = HarvestableAreaOutputsCable$HarvestablePolygons,
                                                       advancedloggingparameters = loggingparameters()))

SecondaryTrails <- secondtrailsopening(
  topography = DTMParacou,
  plotmask = PlotMask,
  maintrails = MainTrails,
...
usethis::use_data(SecondaryTrails, overwrite = TRUE)
```
...
# load(system.file("extdata", "BrokenParacou6_2016.rda", package = "LoggingLab")) # the test inventory in the global environment
