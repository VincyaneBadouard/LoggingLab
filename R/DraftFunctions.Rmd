...
# CrownDiameter formule : CrownDiameter = exp(((log(DBH)-alpha-error)/beta))/TreeHeight
# RottenModel
...
pars <- list(MinDBHValue = 10, #in cm
             ...
             CrownDiameterAllometry = exp(((log(DBH)-alpha-error)/beta))/TreeHeight,
             RottenModel = ) 
```
...
TreeheightGuyafordata %>%
 ggplot() +
  ...
  theme_minimal() +
  facet_wrap(vars(HeightHow), scales = "free_y") #or "fixed"
...
if (!("DBH" %in% names(Paracou6_2016))) {add_column(Paracou6_2016, DBH = NA)
  Paracou6_2016$DBH = Paracou6_2016$CircCorr/pi} # and compute it
...
#continent or region specific model (Feldpaush et al. (2012))
RegionTreeHeight <- retrieveH(D = Paracou6_2016$DBH, region = "GuianaShield") # GuianaShield = Guyana, French Guiana (piste St Elie (Chave 2005)), Venezuela

#by coordinates (Chave et al. (2012))
n <- length(Paracou6_2016$CircCor)
coord <- cbind(long = rep(-52.92, n), lat = rep(5.27, n)) # Paracou P6 coordinates
CoordTreeHeight <- retrieveH(D = Paracou6_2016$DBH, coord = coord) 
