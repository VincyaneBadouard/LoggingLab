...
# pour l'enregistrer : usethis::use_data(BrokenParacou6_2016) puis le move dans ins/exdata
```

# My custom options
faut ptetre mettre Maria. devant  chaque objet!
```{r}
# Mimimum DBH value = 10 cm
...
# Main trail width = 5-6 m
# 2nd trail width = 4 m
# Big trees = 50 cm
# Objective bonus = 20-30%
# Cable = 40 m
# Grapple = 6 m
# Isolate tree minimum distance = 100 m
# Future trees minimum diameter = 35 cm
# TreefallSuccess proportion = 0.6
# Treefall orientation = 30-45◦
# (Hollow) Part taken from hollow trees for fuel exploitation = 1/3
# Purge = 0.14
# Maximum TrailDensity = 200 m/ha
# Maximum landing area = 1 500 m^2


# TrunkHeight formule : TrunkHeight = TreeHarvestableVolume/(pi*(((DBH/100)/2)^2))
# CrownDiameter formule : CrownDiameter = exp(((ln(DBH)-alpha-error)/beta))/TreeHeight
# RottenModel
...
library(BIOMASS)
library(knitr)
# TreeHeight = hauteur de l’arbre (H) (en mètre) (BIOMASS package)
# 3 options :
# - get my tree heights from the location (from continent or region specific model, or from coordinates)
# - from other Guyanese trees heights data, fit different H-D models, select the best and apply on my data.

RegionTreeHeight <- retrieveH(D = Paracou6_2016$CircCor, region = "GuianaShield") #continent or region specific model (Feldpaush et al. (2012))
# GuianaShield = Guyana, French Guiana (piste St Elie (Chave 2005)), Venezuela

n <- length(Paracou6_2016$CircCor)
coord <- cbind(long = rep(-52.92, n), lat = rep(5.27, n)) # Paracou P6 coordinates
CoordTreeHeight <- retrieveH(D = Paracou6_2016$CircCor, coord = coord) #by coordinates (Chave et al. (2012))

result <- modelHD(D = , H = , useWeight = TRUE) #from Guyanese trees heights data
kable(result)
# Compute the local H-D model with the lowest RSE (Root Squared Error)
selectedHDmodel <- modelHD(D = , H = , method = "log2", useWeight = TRUE)

predictHeight(CircCor, model)
