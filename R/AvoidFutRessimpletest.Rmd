...
# 1 on the right side
FutureReserveCrowns[1,]$Xutm <- ind$Xutm + 30
FutureReserveCrowns[1,]$Yutm <- ind$Yutm + 10

# 1 on the left side
FutureReserveCrowns[2,]$Xutm <- ind$Xutm - 30 
FutureReserveCrowns[2,]$Yutm <- ind$Yutm + 10
...
if(TrailPt[1] >= Foot[1]){ # x trail > x foot
  ...
  theta <- 360 - numeric(matlib::angle(c(Foot[1] - Foot[1], ind$TreeHeight),
                                       c(TrailPt[1] - Foot[1], TrailPt[2] - Foot[2]), degree = TRUE))
}
...
if(any(FutureReserveCrowns$TreeHeight <= ind$TreeHeight)){
  
  FutResCrownsPoints <- FutureReserveCrowns %>% 
    filter(TreeHeight <= ind$TreeHeight) %>% 
...
RotationAngle <- c() # empty list for rotation angles
for (FRP in 1:length(FutResAllPoints)) { # for each point
  ...
  if(FutResDist < ind$TreeHeight){
    # sum(FutResDists < ind$TreeHeight) # check av/res trees close to the tree to be felled
    ...
    for (P in 1:length(FootBuffer)) { # for each point of the Foot buffer
      ...
      if(FutResAllPoints[[FRP]][1] >= FootBuffer[[P]][1]){ # x FRP > x foot
        ...
        RotationAngle <- c(RotationAngle, as.numeric(matlib::angle(
          c(FootBuffer[[P]][1] - FootBuffer[[P]][1], ind$TreeHeight), # default position
        ...
        RotationAngle <- c(RotationAngle,  360 - as.numeric(matlib::angle(
          c(FootBuffer[[P]][1] - FootBuffer[[P]][1], ind$TreeHeight), # default position
...
OppAngToAvoid <- RotationAngle - 180 - thetaIndNearest # valeurs négatives c'est là le prblm
...
if(ind$TreeFellingOrientationSuccess == "1"){
