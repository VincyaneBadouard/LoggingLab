...
+ **creekverticaldistance**: Relative vertical distance
...
+ **creekhorizontaldistance**: Relative horizontal distance
(1 m resolution) from nearest channel network
(RasterLayer **with a crs in UTM**) (See \code{\link{CreekDistances}})
To generate creek distances: \code{\link{CreekDistances}} in 'Articles'.
...
(data.frame)("Guide de Sylviculture", ONF ('Office National des Forêts') of French Guiana, 2014)
...
+ **Proportion of the tree crown biomass used as fuel wood** (ref) = 2/3
...
+ **Crown diameter allometry** (Aubry-Kientz et al.2019): ln(D) = 𝜶+ 𝜷 ln(H*CD) + 𝜺,
...
+ **Treefall2ndDeathModel** Estimates the probability of a tree dying when it
is in the area disturbed by the felling of a tree, according to the DBH of
the tree whose probability of dying is estimated.
Default: 1 / (1 + exp(-(-0.47323 + -0.02564 * DBH))) with DBH in cm (developed by M.Rojat)
...
**Description**:
* inventorycheckformat() checks if the input inventory data is compatible with the ‘LoggingLab’ package and returns the inventory if the inventory is in the required format or stop the function if the format is not the one required.
...
Damage:
Secondary windfall: Not all trees under the felled tree (timber or energy) will be considered dead.
The probability of a tree dying under a felled tree is estimated by the model 'Treefall2ndDeathModel',
according to the DBH of the tree whose probability of dying is estimated.
...
DamageBiomass <- FuelV$DamageBiomass # only damage (without purge and hollow trees)
...
+ **FuelWoodBiomass** : Fuel wood biomass in ton (numeric)  
+ **DamageBiomass** : Damages biomass (only damages trees (= not trees logged)) in ton (numeric)  
